...
class Google extends EA_Controller {
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and Proyecto Mapache.
     *
    ...
    public static function sync($provider_id = NULL)
    {
        try
        {
            ...
            foreach ($appointments as $appointment)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new Exception('Event is cancelled, remove the record from Proyecto Mapache.');
                        }

                        // If Google Calendar event is different from Proyecto Mapache appointment then update
                        // Proyecto Mapache record.
                        $is_different = FALSE;
            ...
            // Add Google Calendar events that do not exist in Proyecto Mapache.
            ...
            foreach ($google_events->getItems() as $google_event)
            {
                ...
                // Record doesn't exist in the Proyecto Mapache, so add the event now.
                $appointment = [
     ...
     * documentation of OAuth), every Proyecto Mapache installation should use its own calendar api key. So in every
     * api console account, the "http://path-to-Proyecto Mapache/google/oauth_callback" should be included in an allowed redirect URL.
     */
